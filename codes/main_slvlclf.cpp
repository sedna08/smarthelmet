...
void predict_fdeep_nn(const fdeep::model& slvlclf_model);
 ...
 */
  ...
  gpioSetISRFunc(PIN_BTN, RISING_EDGE, 0, buttonPressCallback);
	...
	processing_thread = thread(predict_fdeep_nn, ref(slvlclf_model));
    ...
    while (f_t1)
    {
        ...
        if (q_cvBuffer[0].size() >= 900)
        {
            ...
            try
            {
                ...
                //       - 0 = awake, 1 = drowsy
                const auto result = slvlclf_model.predict_class({ t }); // predict class
